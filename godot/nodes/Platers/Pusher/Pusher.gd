...
func _on_BodyDetector_body_entered(body):
	if body is RigidBody:
		body.apply_impulse($BodyDetector/CollisionShape.get_global_transform().origin ,pushForce*(($Position3D.get_global_transform().origin - self.get_global_transform().origin).normalized()))
